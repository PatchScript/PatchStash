--[[
This example adds a number of oscillators with each oscillator
having a slightly different pitch. This example demonstrates
how through scripting synthesizers can be created that are
hard to create with traditional paradigmes.
]]--
...
function makeOSC(index, total, voiceFreq)
  ...
  return  (
            (SineWave():freq(voiceFreq * (sineP * shiftAmount)) * sine) +
            (SquareWave():freq(voiceFreq * (squareP * shiftAmount)) * square) +
            (TriangleWave():freq(voiceFreq * (triangleP * shiftAmount)) * triangle)
          ) * 0.33 * gain
end
